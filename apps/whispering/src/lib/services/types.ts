...
/**
 * Callback function for updating status messages during recording operations
 */
...
export type DeviceAcquisitionOutcome =
	...
	| {
			...
			fallbackDevice: DeviceIdentifier;
	  };
...
/**
 * Helper to create a DeviceIdentifier from known sources
 */
export function createDeviceIdentifier(
	value: string | MediaDeviceInfo
): DeviceIdentifier {
	if (typeof value === 'string') {
		return value as DeviceIdentifier;
	}
	// For MediaDeviceInfo, use the deviceId
	return value.deviceId as DeviceIdentifier;
}